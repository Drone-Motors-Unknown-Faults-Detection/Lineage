...
rootDir = os.getcwd()
# 設定儲存數據的目錄
save_base_dir = os.path.join(rootDir, '階段2', 'csv')
...
rawDataDirectories = {
    motor: {
        **{
            screws: os.path.join(rootDir, '階段2', motor, '6000rpm', f'{screws}screws')
            for screws in screws_config
        },
        '1': os.path.join(rootDir, '階段2', motor, '6000rpm', '1screw'),
        '3_14': os.path.join(rootDir, '階段2', motor, '6000rpm', '3_14screws'),
        '4_146': os.path.join(rootDir, '階段2', motor, '6000rpm', '4_146screws'),
    }
...
for motor, directories in rawDataDirectories.items():
    for screws, directory in directories.items():
        ...
        if not all_data.empty:
            save_directory = os.path.join(save_base_dir, motor, '6000rpm', f'{screws}screws')
