...
| Credibility issue 2 | confirmed, pending | Albert README claims one calibration philosophy, but legacy detector calibrates on training distances (`core/mahalanobis.py` legacy path), producing 83.1% healthy false positives | — | formal benchmark must use calibration-only canonical factory methods |
