...
def align_rows(*arrays):
    """
    將所有輸入陣列的行數對齊，以最小行數為基準裁剪。
    """
    min_rows = min(arr.shape[0] for arr in arrays)  # 找到最小行數
    aligned_arrays = [arr[:min_rows, :] for arr in arrays]  # 裁剪行數
...
def remove_outliers(df):
    """
    對每一欄獨立計算 3 倍 IQR，並將每欄的離群值設為 NaN，最後再 drop 掉含 NaN 的 row。
    """
    df_filtered = df.copy()
    for col in df.columns:
        ...
        df_filtered[col] = df[col].where((df[col] >= lower_bound) & (df[col] <= upper_bound), np.nan)
...
def process_group(group_label, screws):
    ...
    group_csv_dir = os.path.join(csvDirectory, group_label, '8000rpm', screws)
    group_feature_dir = os.path.join(myfeatureDirectory, group_label, '8000rpm', screws)
    ...
    try:
        ...
        df_x = pd.read_csv(os.path.join(group_csv_dir, f'{group_label}_Acceleration_X_data.csv'))
        df_y = pd.read_csv(os.path.join(group_csv_dir, f'{group_label}_Acceleration_Y_data.csv'))
        df_z = pd.read_csv(os.path.join(group_csv_dir, f'{group_label}_Acceleration_Z_data.csv'))
        ...
        # 將所有特徵陣列行數對齊
        feature_Current, feature_X, feature_Y, feature_Z, feature_Temp = align_rows(
            feature_Current, feature_X, feature_Y, feature_Z, feature_Temp
        )
        ...
        missing_ratio_Y = feature_data.iloc[:, 41:56].isnull().sum().sum() / (feature_data.shape[0] * feature_data.iloc[:, 41:56].shape[1])
        missing_ratio_Z = feature_data.iloc[:, 66:81].isnull().sum().sum() / (feature_data.shape[0] * feature_data.iloc[:, 66:81].shape[1])
        # 計算電流缺值比例
        missing_ratio_Current = feature_data.iloc[:, 0:15].isnull().sum().sum() / (feature_data.shape[0] * feature_data.iloc[:, 0:15].shape[1])
        # 計算溫度缺值比例
        missing_ratio_Temp = feature_data.iloc[:, 91:106].isnull().sum().sum() / (feature_data.shape[0] * feature_data.iloc[:, 91:106].shape[1])
        print(f"群組 {group_label}，螺絲數量: {screws}，X振動缺值比例: {missing_ratio_X:.8%}")
        print(f"群組 {group_label}，螺絲數量: {screws}，Y振動缺值比例: {missing_ratio_Y:.8%}")
        print(f"群組 {group_label}，螺絲數量: {screws}，Z振動缺值比例: {missing_ratio_Z:.8%}")
        print(f"群組 {group_label}，螺絲數量: {screws}，電流缺值比例: {missing_ratio_Current:.5%}")
        print(f"群組 {group_label}，螺絲數量: {screws}，溫度缺值比例: {missing_ratio_Temp:.8%}")



        # 儲存特徵數據
        output_csv = os.path.join(group_feature_dir, f'{group_label}_Group_feature_data_raw.csv')
